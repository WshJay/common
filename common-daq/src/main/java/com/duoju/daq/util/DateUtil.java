...
public class DateUtil {
	...
	private static DateFormat fullFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	private static DateFormat fullFormat1 = new SimpleDateFormat("yyyy-MM-ddHH:mm:ss");
	private static DateFormat hhmmssSSFormat = new SimpleDateFormat("HHmmssSS");
	private static DateFormat hhFormat = new SimpleDateFormat("HH");
	private static DateFormat chinaFormat=new SimpleDateFormat("d号H点m分");
	private static DateFormat monthFormat=new SimpleDateFormat("yyyy年M月");
	/**
    ...
    public static String getThreeHours(){
    	return fullFormat.format(new Date().getTime()+3*3600*1000);
    }
	/**
	 * 通过字符串创建时间对象
	 * 
	...
	public static String getFullDate1(Date date) {
		return fullFormat1.format(date);
	}
	public static Date parseDate(String shortTime) {
		try {
			return format.parse(shortTime);
		} catch (ParseException e) {
			e.printStackTrace();
		}
		return null;
	...
	public static Date parse(String fullTime) {
		try {
	...
	public static String getHHmmssSSFormat(Date date) {
		return hhmmssSSFormat.format(date);
	}
	/**
	 * 获取今日开始时间
	 * @return
	...
	public static void main(String[] args) {
		String s=fullFormat.format(new Date());
		String s1=getTodayBegin();
		String S2=getTodayEnd();
		String s3=getTomorrowBegin();
		String s4=getWeekBegin();
		String s5=getThreeHours();
	...
	public static String getTodayEnd(){
		String timeStr=format.format(new Date())+" 23:59:59";
		return timeStr;
	}
	...
	public static String getTomorrowBegin(){
		long daySecond=24*3600L;
			return format.format(new Date(new Date().getTime()+daySecond*1000))+" 00:00:00";
	}
	/**
	 * 获取明日结束时间
	 * @return
	...
	public static String getTomorrowEnd(){
		long daySecond=24*3600L;
		return format.format(new Date(new Date().getTime()+daySecond*1000))+" 23:59:59";
	}
	 ...
	 * @return
	 */
	public static String getAfterTomorrowBegin(){
		long daySecond=2*24*3600L;
		return format.format(new Date(new Date().getTime()+daySecond*1000))+" 00:00:00";
	}
	/**
	 * 获取后天结束时间
	...
	public static String getWeekBegin(){
		...
		String beginTimeStr=format.format(cal.getTime());
		return beginTimeStr+" 00:00:00";
	}
	...
	/**
	 * 获取指定日期所在下个星期的开始时间（周一早上零时）
	 * 
	 * @param date
	 * @return 格式：2010-09-31 00:00:00
	 */
	public static String getNextWeekBegin(Date date) {
		Calendar cal = new GregorianCalendar();
		cal.setTime(date);
		// 代表今天是星期几，周日---周六 返回 1---7
		int today = cal.get(Calendar.DAY_OF_WEEK);
	...
	/**
	 * 获取指定日期所在下下个星期的开始时间（周一早上零时）
	 * 
	 * @param date
	 * @return 格式：2010-09-31 00:00:00
	 */
	public static String getNextTwoWeekBegin(Date date) {
		Calendar cal = new GregorianCalendar();
		cal.setTime(date);
		// 代表今天是星期几，周日---周六 返回 1---7
		int today = cal.get(Calendar.DAY_OF_WEEK);
		...
		if (1 == today) {
			...
			cal.add(Calendar.DATE, -today + 16);
		}
		cal.set(cal.get(Calendar.YEAR), cal.get(Calendar.MONTH),
				cal.get(Calendar.DATE), 0, 0, 0);
		return longDate(cal.getTime());
