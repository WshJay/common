...
// 电话验证：包括 手机和固定电话 CXW 2010-03-19
function isphone1(inpurStr){
     var partten = /(0[1-9]{1}[0-9]{1,2})[-]?[2-8]{1}[0-9]{6,7}$/;	// 固定电话
     var parttentest= /^1[3|5|8]{1}[0-9]{1}[0-9]{8}$/;				// 手机号
     if(inpurStr.length == 11){
     	return parttentest.test(inpurStr);
     }else{
     	return partten.test(inpurStr);
     }
     return false;
}
...
/*
 * 用途：检查输入对象的值是否符合MSN格式 输入：str 输入的字符串 返回：如果通过验证返回true,否则返回false
 */  
function isMsn(str)
{
	if(isNull(str)) return false;
    var msnre = new RegExp(/^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/g);
    if(msnre.test(str)){
	   return true;
    }else{
	   return false;
...
function checkPhone( strPhone ) {    
 var phoneRegWithArea = /^[0][1-9]{2,3}-[0-9]{6,10}$/;    
...
function f_checkTextValid(obj){   
 ...
 /*
	 * switch(checkType){ case "number": if(f_isNumber(obj) == false) return
	 * false;break; case "date": if(f_checkDate(obj) == false) return
	 * false;break; default: }
	 */  
 return true;   
}   
...
/*
 * function selectedCheckboxCount(form) 功能说明：对Form中选中的可选项计数
 * 
 * 参数说明： form:指定的表单
 */  
...
/*
 * 说明： 1.清除表格中的数据(0.0 和 0) 2.如果cell中没有数据，则自动加上一个空格 3.清除空白行的checkbox
 * 
 * 参数： clearzero:是否清除"0"、"0.0"，false不清除、true清除（默认为true）
...
/*
 * 用途：检查输入字符串是否是带小数的数字格式,可以是负数(并且满足规定的精度) 输入：str：字符串 l：总位数 d：小数点后位数 返回：
 ...
 * 设置文章编辑器编辑模式为可编辑
 */	
function setArticleEditMode()
{
	if(typeof(eWebEditor1)=="undefined")
		return;
	if("" !=eWebEditor1 && null !=eWebEditor1)
	{
